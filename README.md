...
Istrech =imadjust(I,stretchlim(I)); %Adjust image intensity values or colormap.
...
The major constraint in the project is the angle in which the image of the track is captured. The alternatives for this project are drone images can be captured which can capture precise images at specific angles. Another major constraint is that the images should be captured in such a way that the image majorly focusses on the track with the track, background details of the ground stones or other fillers may hamper the efficiency of image processing of the model. The cleanliness of the track is also one of the requirements. Tradeoffs in this project is finding the optimum image from a huge dataset, to get the maximum results.

**PROJECT DEMONSTRATION**

I have attached the step-by-step photos of all the design:
...
My project would only involve costs for installation of the camera and for the computer vision system. The camera would cost about 1k-2.5k and the cost of procuring a computer vision system would be approximately 15k. Therefore, the project is cost effective as it reduces expenditure on labor costs. The program will run on MATLAB software and will provide results on the screen. Laptop (40k) Total Cost: Practical purposes: Rs. 56-57.5KIn case of simulation we take a dataset of images of railway tracks available on the internet. According to various surveys, a computer vision investment can be recovered in less than a year. Also, compared to the economic damage and the loss of lives that faults in railway tracks can cause, the costs incurred are reasonable.To asses if the model is successful:

•Once the crack is being identified, the crack length is noted. 

•Then a team is sent to analyze the crack.

•On inspecting the crack, the team decides to either weld the crack if the crack is not severe.

•In severe cases where there could be a derailment, the whole part has to be changed.

•Minute cracks will be identified through image processing which cannot be seen through the naked eye. If the model fails to identify the crack at the first go, the model is run several times on the image to get a better accuracy.


Result and Discussion

Various photographs of tracks are taken and the original image with crack is processed in MATLAB. If crack is found the image is contrast stretched and RGB to gray image is obtained. A median filtered image is obtained by applying median filter. A BOT HAT output image of the crack of the track is obtained and on binary image noise reduction techniques are applied via noise filter. Thinned image or skeleton image of crack is visualized, and crack length is also calculated and displayed. This proposed crack detection algorithm can detect the cracks on the tracks and calculate the crack length. The value of crack length can be used to measure the severity of crack and life of crack.    

The obtained images of MATLAB are shown in Fig.4 to Fig.12. My project would only involve costs for installation of the camera and for the computer vision system. The camera would cost about 1k-2.5k and the cost of procuring a computer vision system would be approximately 15k. Therefore, the project is cost effective as it reduces expenditure on labor costs. The program will run on MATLAB software and will provide results on the screen. In case of simulation, we take a dataset of images of railway tracks available on the internet. According to various surveys, a computer vision investment can be recovered in less than a year. Also, compared to the economic damage and the loss of lives that faults in railway tracks can cause, the costs incurred are reasonable. For the image used, we have found the crack length is 209.06 cm.

**SUMMARY**

I have performed a detailed literature survey. Collected the images of the faulty tracks and modified the images for identification of the crack. Removed all the background details and focused only on the image of the track. We identified all the cracks, applied different filters to get a clear image. We located the crack in the image, fine-tuned the image for better crack length estimation.
I have converted the image into pixels and estimated the crack length and the gap length. At the final stage we displayed all the images at different stages and the crack length in the modified image. The identified crack length can be used by inspection officers to estimate the risk whether the tracks could either be replaced or welded to prevent derailments and accidents. Further we found the system very cost effective also.
